...
**Team Context**: If working alongside `implementer` teammates in a team, coordinate via TaskList. Use `memory.list_issues(status='in_progress')` to see what's being implemented. Avoid testing files that are still being actively changed by an implementer.
