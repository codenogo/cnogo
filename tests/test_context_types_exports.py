...
This file covers process_types from scripts.context.phases.types.
"""
...
from scripts.context.parser_base import ParseResult, SymbolInfo, TypeRef
    ...
    parse_results = {
        "utils.py": ParseResult(
            type_refs=[
                ...
                TypeRef(name="string", kind="annotation", line=9),
                TypeRef(name="number", kind="annotation", line=10),
            ],
        )
    }
    storage = _setup_graph(tmp_path, files, parse_results)
    ...
    parse_results = {
        "svc.py": ParseResult(
            ...
            type_refs=[
                ...
                TypeRef(name="Config", kind="annotation", line=15),
            ],
        )
    }
    storage = _setup_graph(tmp_path, files, parse_results)
...
def test_types_stores_kind_property(tmp_path):
    """Relationship properties contain the kind of the TypeRef."""
    from scripts.context.phases.types import process_types
