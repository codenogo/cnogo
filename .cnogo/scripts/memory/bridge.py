...
def generate_implement_prompt(
    ...
    *,
    ...
    if task_id:
        ...
        lines.append("")
        lines.append("**On success:** Add these THREE final lines (in order):")
        lines.append("1) `TASK_RESULT: <single-line JSON>` — fields: status(\"done\"), taskId, error(null), errorCategory(null), filesModified(list), verifyResults([{cmd,passed,output}]), retryCount(int)")
        lines.append("2) `TASK_EVIDENCE: <single-line JSON>`")
        lines.append(f"3) `{completion_footer}`")
        ...
        lines.append("")
        lines.append("**On failure/blocked:** Emit ONLY `TASK_RESULT: <JSON>` with status \"failed\" or \"blocked\", error message, errorCategory (verify_fail/blocked_dependency/file_conflict/timeout/unknown). Do NOT emit TASK_EVIDENCE or TASK_DONE.")
    lines.append("")
