# Plan 02: Add two-phase completion API (report_done + verify_and_close) with role enforcement and kill auto-close-parent (Contracts 02, 04, 05)

## Goal
Add two-phase completion API (report_done + verify_and_close) with role enforcement and kill auto-close-parent (Contracts 02, 04, 05)
...
python3 -c "import sys; sys.path.insert(0,'.'); from scripts.memory import report_done; print('report_done importable')"
```
...
```bash
python3 -m py_compile scripts/memory/__init__.py
python3 -c "import sys; sys.path.insert(0,'.'); from scripts.memory import verify_and_close; print('verify_and_close importable')"
```
...
python3 -c "import sys; sys.path.insert(0,'.'); from scripts.memory import __init__ as m; assert not hasattr(m, '_try_auto_close_parent'), 'auto-close-parent still exists'; print('OK')"
```
...
## Commit Message
```
feat(deterministic-coordination): add two-phase completion API and kill auto-close-parent cascade
```
