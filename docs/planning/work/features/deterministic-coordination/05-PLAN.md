...
python3 scripts/workflow_validate.py --help 2>&1 | grep -q baseline || python3 scripts/workflow_validate.py 2>&1; echo "exit: $?"
```
...
```bash
python3 -m py_compile scripts/workflow_validate.py
python3 scripts/workflow_validate.py --save-baseline 2>&1; python3 scripts/workflow_validate.py --diff-baseline 2>&1; echo "exit: $?"
```
