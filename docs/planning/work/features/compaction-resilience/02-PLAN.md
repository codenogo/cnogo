...
**Files:** `.cnogo/hooks/hook-pre-compact.py`
**Action:**
...
**Action:**
Add a new `PreCompact` hook entry in `.claude/settings.json` under the `hooks` key. Add it after the `SubagentStop` entry. Structure: `"PreCompact": [{"hooks": [{"type": "command", "command": "python3 .cnogo/hooks/hook-pre-compact.py"}]}]`. Keep all existing hooks unchanged.
