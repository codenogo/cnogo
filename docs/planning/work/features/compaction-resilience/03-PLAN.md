# Plan 03: Wire reconcile into CLI, export public API, and update recovery commands

## Goal
Wire reconcile into CLI, export public API, and update recovery commands
...
**Verify:**
```bash
python3 -c "text=open('.claude/commands/resume.md').read(); assert 'session-reconcile' in text, 'resume.md missing session-reconcile'"
```
...
**Verify:**
```bash
python3 -c "text=open('.claude/CLAUDE.md').read(); assert 'session-reconcile' in text, 'CLAUDE.md missing session-reconcile'"
```
...
## Commit Message
```
feat(compaction-resilience): wire session-reconcile CLI, update resume and docs
```
