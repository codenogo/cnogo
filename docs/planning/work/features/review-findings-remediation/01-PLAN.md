...
**Files:** `scripts/hook-dangerous-cmd.sh`, `scripts/hook-sensitive-file.sh`, `scripts/hook-pre-commit-secrets.sh`
**Action:**
Extract the 3 PreToolUse hooks from `.claude/settings.json` into standalone shell scripts:

1. **`scripts/hook-dangerous-cmd.sh`** — Dangerous command blocker (from settings.json line 12).
...
2. **`scripts/hook-sensitive-file.sh`** — Sensitive file reader blocker (from settings.json line 21).
   - Use `printf '%s'` for safe input handling.

3. **`scripts/hook-pre-commit-secrets.sh`** — Pre-commit secret scanner (from settings.json line 30).
...
**Verify:**
```bash
bash -n scripts/hook-dangerous-cmd.sh
bash -n scripts/hook-sensitive-file.sh
bash -n scripts/hook-pre-commit-secrets.sh
...
1. Replace the 3 inline PreToolUse hook commands with calls to the extracted scripts:
   - Dangerous cmd: `bash scripts/hook-dangerous-cmd.sh`
   - Sensitive file: `bash scripts/hook-sensitive-file.sh`
   - Pre-commit secrets: `bash scripts/hook-pre-commit-secrets.sh`
2. **Remove the pre-commit test runner hook entirely** (the second Bash hook at line 33-34) (finding #5).
3. **Fix PostToolUse hook path**: Change `python3 $HOME/.claude/scripts/workflow_hooks.py post_edit` to `python3 scripts/workflow_hooks.py post_edit` (finding #2).
...
bash scripts/hook-pre-commit-secrets.sh
```
...
bash -n scripts/hook-dangerous-cmd.sh && bash -n scripts/hook-sensitive-file.sh && bash -n scripts/hook-pre-commit-secrets.sh
# settings.json valid
