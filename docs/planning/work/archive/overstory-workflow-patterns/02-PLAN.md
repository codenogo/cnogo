# Plan 02: Add transcript-based cost tracking with model-specific pricing and memory event storage

## Goal
Add transcript-based cost tracking with model-specific pricing and memory event storage
...
**Verify:**
```bash
python3 -m py_compile scripts/memory/costs.py
python3 -c "from scripts.memory.costs import PRICING, estimate_cost; assert 'opus' in PRICING; assert estimate_cost.__doc__ or True"
```
...
**Verify:**
```bash
python3 -c "import scripts.memory as m; assert 'record_cost_event' in m.__all__"
```
...
## Commit Message
```
feat(costs): add transcript-based cost tracking with model-specific pricing
```
