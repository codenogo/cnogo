...
- Run `python3 scripts/workflow_validate.py` before shipping workflow changes.
- Use memory engine APIs (`scripts/memory/`) instead of ad-hoc state files.

## Structure

- `scripts/memory/` memory engine (SQLite + JSONL sync)
- `scripts/workflow_checks.py` package-aware review/verify + token telemetry/discover
- `scripts/workflow_validate.py` workflow contract + freshness + budget validation
- `scripts/workflow_hooks.py` post-edit formatting and pre-bash optimization telemetry
